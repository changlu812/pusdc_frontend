...
async function updateBalance() {
  ...
  try {
    ...
    console.error("Error updating claimable balance:", err);
  }
}
...
async function handleAction() {
  ...
  const apiUrl = `${PUSDC_API}/api/zentra/usdc/sign_deposit?addr=${account}&amount=${parsedAmount.toString()}&nonce=${(nonce + 1n).toString()}&balance=${balance || "0x"}`;
  ...
  // 3. Call privacyDeposit via instruction
  ...
  // 已在上面 Promise.all() 中并行执行，这里只需更新余额
  await updateBalance();
